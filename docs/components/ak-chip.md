...
Pass `@onDelete` to render the delete icon; the handler receives the click `MouseEvent` (propagation is stopped before it reaches the chip). Use `@button={{true}}` when the chip should behave as a button.
