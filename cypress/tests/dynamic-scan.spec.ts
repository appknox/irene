...
const DynamicStatusTexts = [
  APP_TRANSLATIONS.deviceBooting,
  APP_TRANSLATIONS.deviceInstalling,
];
...
describe('Dynamic Scan', () => {
  beforeEach(() => {
    ...
    cy.intercept(API_ROUTES.projectList.route).as('orgProjectListReq');
    cy.intercept(API_ROUTES.submissionList.route).as('submissionList');
  });

  APP_TYPE_DETAILS.forEach((app) =>
    it(
      `it tests dynamic scan for an ${app.type} file`,
      ...
      () => {
        ...
        cy.wait('@orgProjectListReq', NETWORK_WAIT_OPTS);

        // get current project from project list response
        dynamicScanActions
          .getProjectResponseForAppInfo(app, '@orgProjectListReq')
          .as('currentProject');

        // use current project to assert & get project root element
        cy.get<MirageFactoryDefProps['project']>('@currentProject').then(
          (prj) => {
            const lastFileId = prj.last_file_id;

            cy.wrap(prj.package_name).should('be.eq', app.packageName);

            // assert current project
            cy.findByTestId(`project-overview-${lastFileId}`)
              .should('exist')
              .within(() => {
                cy.findByRole('heading', { name: app.name }).should('exist');

                cy.findByText(lastFileId).should('exist');
                cy.findByText(prj.package_name).should('exist');
              })
              .as('currentProjectEle');

            // interceptor for project click
            cy.intercept(`${API_ROUTES.file.route}/${lastFileId}`).as(
              'currentProjectFileReq'
            );
          }
        );

        // route to file page
        cy.get('@currentProjectEle').click({ force: true });

        // Check if in file page
        cy.url().should('contain', APPLICATION_ROUTES.file);

        // save last file response & verify app name
        cy.get('@currentProjectFileReq')
          .its('response.body')
          .as('currentProjectFile')
          .its('name')
          .should('match', app.name);

        // do file page assertions
        cy.get<MirageFactoryDefProps['file']>('@currentProjectFile').then(
          (file) => {
            // sanity check
            ...
            cy.findByTestId(
              ...
              cy.findByText(APP_TRANSLATIONS.dynamicScan, assertOpts).should(
                'exist'
              );

              if (file.is_dynamic_done) {
                cy.findByRole('button', {
                  name: APP_TRANSLATIONS.completed,
                }).should('exist');
                ...
                cy.findByRole('button', { name: APP_TRANSLATIONS.start })
                  .should('exist')
                  .as('startRestartdynamicScanBtn');
              }
            });
            ...
            cy.findByTestId('dynamicScanModal').within(() => {
              ...
              cy.findByRole('heading', {
                name: APP_TRANSLATIONS.dynamicScan,
              }).should('exist');
              ...
              cy.get('@devicePreferenceReq')
                ...
                .then(({ device_type: type, platform_version: version }) => {
                  ...
                  if (type !== 0) {
                    ...
                    cy.document()
                      .findByRole('option', {
                        name: APP_TRANSLATIONS.anyDevice,
                      })
                      .should('exist')
                      .click({ force: true });
                    ...
                    cy.document()
                      .findByText(APP_TRANSLATIONS.savedPreferences)
                      .should('exist');
                  }
                });

              cy.findByRole('button', {
                name: APP_TRANSLATIONS.modalCard.dynamicScan.start,
              })
            ...
            cy.findByTestId('dynamicScan-infoContainer').within(() => {
              ...
              cy.findByRole('button', {
                name: APP_TRANSLATIONS.stop,
                timeout: statusTimeout,
              })
                .should('exist')
                .as('stopDynamicScanBtn');
            });
            ...
            dynamicScanActions.interactWithApp(app.interactions, app);
          }
        );
      }
    )
  ...
  afterEach(() => {
    // stop dynamic scan
    cy.get('@stopDynamicScanBtn').click({ force: true });

    // dynamic scan btn while stoping
    cy.findByTestId('dynamicScan-infoContainer')
      .findByRole('button', {
        name: APP_TRANSLATIONS.deviceShuttingDown,
      })
      .should('exist');
  });
});
