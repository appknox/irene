...
describe('Organization Users', () => {
  ...
  it('should add/remove user to team', function () {
    ...
    // used for clean up in afterEach
    cy.get('@teamIncludesUser').then((interception: any) => {
      const orgId = interception.response?.body?.results[0]?.organization;
      const teamId = interception.response?.body?.results[0]?.id;

      // Store `orgId` and `teamId` for cleanup
      cy.wrap(orgId).as('orgId');
