...
export default class UploadAppActions {
  ...
  selectAppViaSystem(fileFixtureAlias: string) {
    ...
    return cy
      .findByRole('button', {
        name: APP_TRANSLATIONS.uploadApp,
      })
      .then((el) => {
  ...
  selectAppViaLink(playStoreUrl: string) {
    ...
    cy.findByLabelText('upload app via link button').click();

    cy.findByText(
      APP_TRANSLATIONS.uploadAppModule.linkUploadPopupHeader
    ).should('exist');

    cy.findByPlaceholderText(
      APP_TRANSLATIONS.uploadAppModule.linkPastePlaceholder
    ).should('not.have.attr', 'disabled');
    ...
    cy.findByLabelText('upload app via link modal confirm button').should(
      'not.have.attr',
      'disabled'
    );

    cy.findByLabelText('upload app via link modal confirm button').click();
  }

  waitForSBOMProjectList(
    sbomProjectListAlias: string,
    uploadedAppPrjIDAlias: string,
    uploadedFileDetailsAlias: string,
    uploadedAppPackageNameAlias: string
  ) {
    cy.wait(sbomProjectListAlias, { timeout: 6000 }).then(
      ({ response: res }) => {
  ...
  checkStaticScanInfo(file: MirageFactoryDefProps['file']) {
    cy.findByTestId('staticScan-infoContainer', { timeout: 30000 }).then(
      (el) => {
        ...
        cy.findByText(APP_TRANSLATIONS.staticScan, assertOpts).should('exist');

        // Check if static scan has completed or is in progress
        if (file.is_static_done) {
          cy.findByText(APP_TRANSLATIONS.completed, assertOpts).should('exist');
  ...
  initiateViaLinkUpload = (url: string) => {
    ...
    cy.findByTestId('submission-status-dropdown-container', {
      ...
      cy.contains(APP_TRANSLATIONS.uploadStatus);
      cy.contains(APP_TRANSLATIONS.viaLink);
    });

    // Initiate link upload
    cy.wait('@uploadAppLinkReq').then(({ response: res }) => {
      expect(res?.statusCode).to.equal(201);

      const uploadDetails =
        res?.body as MirageFactoryDefProps['upload-app-link'];

      this.openUploadAppModal();

      cy.findByText(APP_TRANSLATIONS.viaLink).should('exist');

      cy.findByText(APP_TRANSLATIONS.uploadStatus).should('exist');

      cy.findByTestId(`upload-app-status-details-${uploadDetails.id}`, {
        timeout: 30000,
      });
    });
