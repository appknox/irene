...
export default class SbomComponentModel extends Model {
  ...
  get confidenceLabel() {
    if (!this.aiConfidence) {
      return null;
    }
    const labelMap: Record<string, string> = {
      high: 'sbomModule.confidenceLevel.high',
      medium: 'sbomModule.confidenceLevel.medium',
      low: 'sbomModule.confidenceLevel.low',
    };
    const key = labelMap[this.aiConfidence];
    return key ? this.intl.t(key) : null;
  }

  get confidenceColor() {
    if (this.aiConfidence === 'high') {
      return 'success';
    }
    if (this.aiConfidence === 'medium') {
      return 'warn';
    }
    return 'default';
  }
  ...
  get confidenceExplanationKey() {
    if (!this.aiArtifactClass || !this.aiConfidence) {
      return null;
    }
    ...
    const explanationMap: Record<string, string> = {
      ...
      'cloud_endpoint:medium':
        'sbomModule.confidenceExplanation.cloudGenericMatch',
   ...
   * kind is only ever surfaced via the "Purpose" field (aiPurposeDisplay),
   * not repeated in this column.
   */
  get aiTypeLabel() {
  ...
  /**
   * Display value for the "Purpose" field, shared by the AI BoM drawer and
   * the component-details summary so the two can't show different purposes
   * for the same component. Prefers the backend's ai_purpose, then
   * aiModelCategory (for "model", where it's more specific than the generic
   * fallback -- e.g. "Image Classification" vs "General AI Model"), then
   * aiPurposeFallback.
   */
