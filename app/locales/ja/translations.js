export default {
  "acceptInvite": "招待を受けてサインアップする",
  "acceptTerms": "私は利用規約に同意します",
  "accessPermissions": "Access permissions",
  "accessRequested": "Removal for the restriction has been requested, please wait for our team to revert",
  "account": "アカウント",
  "accountActivated": "Your account has been activated. Please login to continue",
  "accountOverview": "アカウントの概要",
  "accountSettings": "アカウント設定",
  "actions": "アクション",
  "activationSuccess": "アカウントが有効化されていると、ログインページにリダイレクトされます。",
  "activeSubscription": "有効なサブスクリプション",
  "addCollaborator": "Add Collaborator",
  "addedNamespaces": "Added Namespaces",
  "addedOn": "Added on",
  "addedSSOMembersToOrg": "Created members from SSO emails",
  "addMember" : "Add Member",
  "addMembers" : "Add Members",
  "addNamespace": "名前空間の追加",
  "addNewNamespace": "以下に名前空間が追加できます",
  "addNewTeam": "新しいチームの追加",
  "addProject" : "Add Project",
  "addSSOMembers": "Add SSO members",
  "addTags": "Add tags",
  "addTeam": "チームの追加",
  "admin": "管理者",
  "adminPermissionProject": "Admin permission is required to connect project to JIRA",
  "allFutureAnalyses": "All Future Analyses",
  "allowEdit": "Allow Edit",
  "allPlatforms": "全プラットフォーム",
  "allScans": "全診断",
  "allUploads": "全アップロード",
  "alreadyHaveAccount": "すでにアカウントをお持ちですか？",
  "amountPaid": "支払い金額",
  "analysisSettings": "Analysis Settings",
  "analysisUpdated": "Analysis Updated",
  "analytics": "Analytics",
  "anyDevice": "Any Device",
  "anyVersion": "Any Version",
  "api": "API",
  "apiScan": "API診断",
  "apiURLFilter": "API URLフィルタ",
  "app": "アプリ",
  "approvalStatus": "Approval",
  "approve": "Approve",
  "apps": "アプリ",
  "assignTeam":	"Assign Team",
  "analyzing": "分析中",
  "attachments": "添付ファイル",
  "back": "Back",
  "backToTeams": "Back to teams",
  "billing": "料金プラン",
  "billingPeriod": "支払い請求周期",
  "businessImplication": "ビジネスへの影響",
  "by": "by",
  "cancel": "キャンセル",
  "cancelSubsciption": "サブスクリプションのキャンセル",
  "capturedApiTitle": "Captured APIs",
  "capturedApiEmptyTitle": "No APIs captured during the last dynamic scan. Please try again",
  "capturedApiEmptyDesc": "1. Make sure you go through all views of the app during the dynamic scan<br>2. Check if correct API URL filters are set<br>3. Ensure SSL pinning is not enabled in app",
  "changeDevice": "デバイスの変更",
  "changePassword": "パスワードの変更",
  "changeRole": "役割の変更",
  "chooseAFile": "比較するファイルを選択してください",
  "clickingHere": "clicking here",
  "clickOnDynamicScan": "動的診断の開始をクリック",
  "clickToAddOrgName": "Click to add organization name",
  "close": "Close",
  "closeModal": "全画面を終了",
  "collaboratorRemoved": "Removed Collaborator",
  "collaborators": "Collaborators",
  "commaSeparatedEntries": "(for multiple entries, separate by comma)",
  "comment": "Comment",
  "companyName": "会社名",
  "compare": "比較",
  "compareWarningSameFiles": "You are comparing 2 same files",
  "compareWarningOldFile": "You are comparing an old file with new file, the status below will not be accurate.",
  "compareSummary1": "You are comparing file of ID",
  "compareSummary2": "with file of ID",
  "completed": "完了しました",
  "confirmPassword": "新しいパスワード(確認)",
  "continue": "続ける",
  "copy": "コピー",
  "createAccount": "Create Account",
  "created": "作成されました",
  "createdOn": "作成日",
  "createTeam": "チームの作成",
  "credentialsNotUpdated": "資格情報の更新中に問題が発生しました",
  "credentialsUpdated": "資格情報が更新されました",
  "critical": "クリティカル",
  "currentFileOnly": "Current File Only",
  "currentPassword": "現在のパスワード",
  "currentPlan": "現在のプラン",
  "cvssMetrics": "CVSS Metrics",
  "date": "日付",
  "dateCreated": "作成日",
  "dateUpdated": "更新日",
  "delete": "削除",
  "deleteInvitation": "delete invitation",
  "deleteTeam": "チームを削除",
  "description": "説明",
  "details": "詳細",
  "developer": "開発者",
  "developerSettings": "開発者向け設定",
  "devicefarm": "Devicefarm",
  "deviceBooting": "起動中",
  "deviceInQueue": "順番待ち",
  "deviceDownloading": "ダウンロード中",
  "deviceInstalling": "インストール中",
  "deviceLaunching": "開始中",
  "deviceHooking": "フック中",
  "deviceShuttingDown": "停止中",
  "deviceCompleted": "完了しました",
  "deviceIsReady": "デバイスの接続準備ができました",
  "deviceNotFound": "デバイスが見つかりませんでした",
  "deviceSelected": "デバイスを選択しました",
  "deviceType": "デバイスタイプ",
  "devicePreferences": "デバイスの設定",
  "disabled": "無効",
  "disableMFA": "MFAの無効化",
  "disconnect": "Disconnect",
  "download": "ダウンロード",
  "downloadAppTitle": "Download App",
  "dynamic": "動的",
  "dynamicScan": "動的診断",
  "dynamicScanStart": "動的診断を開始",
  "dynamicScanStop": "動的診断を停止",
  "dynamicScanText": "Please refresh the page if the scan doesn't start in 1-2 mins",
  "dynamicShutDown": "動的診断を終了",
  "edit": "Edit",
  "editAnalysis": "Edit Analysis",
  "editAnalyses": "Edit Analyses",
  "email": "Eメール",
  "emailOrUsername": "Eメール/ユーザ名",
  "emptyDescription": "Please fill the description",
  "emptyEmailId": "メールIDを入力してください",
  "emptyURLFilter": "Enter host name",
  "enabled": "有効",
  "enableMFA": "MFAの有効化",
  "enterFileID": "Enter the file ID",
  "enterNamespace": "Enter the namespace",
  "enterOTP": "6桁のOTPを入力してください",
  "enterRightTeamName": "削除するチーム名を正確に入力してください",
  "enterRightUserName": "削除するユーザ名を正確に入力してください",
  "enterTeamName": "チーム名を入力してください",
  "enterTokenName": "トークン名を入力してください",
  "enterValidPassword": "有効なパスワードを入力してください",
  "enterValidURL": "有効なURLを入力してください",
  "errorWhileFetching": "署名付きURLの取得時にエラー",
  "errorWhileUploading": "事前署名付きURLへのファイルのアップロード中にエラー",
  "errorOccured": "Error Occured:",
  "expiring": "期限切れ",
  "expired": "期限切れ",
  "failedGitHubProject": "Something went wrong in getting the integrated project.",
  "falsePositive": "Is this a false positive? you can override this analysis here",
  "fetchGitHubRepoFailed": "リポジトリリストの取得時に問題が発生しました",
  "fetchProjectFailed": "プロジェクトリストの取得時に問題が発生しました",
  "findingAdded": "Finding Added",
  "fileCount": "ファイル数",
  "fileDetails": "ファイルの詳細",
  "files": "ファイル",
  "fileUploadedSuccessfully": "ファイルがアップロードされました。ファイルを処理していますのでお待ちください。",
  "filterBy": "フィルタリングする",
  "first": "最初",
  "for": "for",
  "forgotPassword": "パスワードを忘れました",
  "general": "一般",
  "generalSettings": "General Settings",
  "ghPageConfigure": "GitHub連携設定ページ",
  "githubIntegration": "GitHub連携",
  "githubWillBeRevoked": "GitHub認証は直ちに取り消されます",
  "githubErrorIntegration": "Inter failed, please contact support",
  "githubIntegratedSuccessful": "Successfully integrated GitHub with user",
  "gotoHome": "ホームに戻る",
  "gotoSettings": "is not integrated, you can go to settings &amp; integrate by",
  "greet": "サービスのセキュリティファナティック",
  "halfYearly": "６か月",
  "halt": "診断を停止して、サポートに連絡してください",
  "hangInThere": "URLを処理する間、そのままお待ちください",
  "hiddenAnalysis": "HIDDEN ANALYSIS",
  "hideVulnerability": "脆弱性を非表示にする",
  "high": "高",
  "howAndWhat": "どのように？",
  "host": "Host",
  "identification": "ユーザ名/Eメール",
  "impact": "Impact",
  "improved": "改善",
  "integrateGithub": "GitHubと連携する",
  "integratedGithub": "このプロジェクトはGitHubリポジトリと連携されています",
  "integrateJIRA": "JIRAと連携する",
  "integratedJIRA": "このプロジェクトはJIRAプロジェクトと連携されています",
  "integration": "連携",
  "integrationCompleted": "連携が完了しました",
  "invalidAccountDetails": "Please enter valid account details",
  "invalidUsernameOrEmail": "PLease enter valid Username or Email",
  "invalidUsernameOrPassword": "PLease enter vaid Username or Password",
  "invalidPassword": "パスワードが正しくありません",
  "invalidURL": "は無効なURLです",
  "inValidCredentials": "Invalid credentails",
  "invalidCVSS": "Invalid CVSS metrics",
  "invite": "招待",
  "inviteMember": "Invite Member",
  "inviteType": "Invite type",
  "invitationDeleted": "Invitation is deleted successfully",
  "invitationReSent": "Invitation re-sent successfully",
  "invitedOn": "Invited on",
  "invoice": "請求書",
  "invoices": "請求書",
  "invoiceId": "請求書ID",
  "jiraHost": "JIRAホスト",
  "jiraIntegrated": "JIRAと連携されました",
  "jiraIntegration": "JIRA連携",
  "jiraPageConfigure": "JIRA連携設定ページ",
  "jiraWillBeRevoked": "JIRA認証は直ちに取り消されます",
  "jiraNoProject": "Linked JIRA account doesn't have any projects",
  "language": "言語",
  "last": "最終",
  "lastSixMonths":"Last 6 months",
  "leastRecent": " (古い順)",
  "lessDetails": "less details",
  "loading": "読み込み中",
  "login": "ログイン",
  "loginFailed": "Login failed",
  "loginTitle": "Member Login",
  "logout": "ログアウト",
  "low": "低",
  "manager": "マネージャー",
  "manual": "手動",
  "manualRequested": "手動診断がリクエストされました",
  "manualScan": "手動診断",
  "markAnalysis": "Mark Analysis",
  "markItAs": "Mark it as",
  "medium": "中",
  "member": "メンバー",
  "memberCount": "Members count",
  "memberRole": "メンバー",
  "members": "メンバー",
  "mfaEnabled": "マルチファクタ認証が有効になりました",
  "mfaDisabled": "マルチファクタ認証が無効になりました",
  "mfaDescription": "マルチファクタ認証はアカウントのセキュリティを強化します。",
  "month": "月",
  "monthly": "１か月",
  "mostRecent": " (新しい順)",
  "moreDetails": "more details",
  "multiFactorAuth": "マルチファクタ認証",
  "name": "名前",
  "namespace": "名前空間",
  "namespaceApproved": "Namespace approved successfully",
  "namespaceRejected": "Namespace rejected successfully",
  "namespaces": "名前空間",
  "networkError": "ネットワークエラーが発生しました。再度お試しください。",
  "newPassword": "新しいパスワード",
  "newScan": "新規診断",
  "newScanStarted": "New scan started, Click here to show",
  "next": "次",
  "no": "いいえ",
  "noCollaborators": "No Collaborators",
  "noDeviceAvailable": "使用可能なデバイスがありません",
  "noFiles": "ファイルがありません",
  "noMembers": "No members",
  "noNamespace": "名前空間がありません",
  "none": "なし",
  "on": "on",
  "off": "off",
  "passed": "合格",
  "noInvitations": "No Pending Invitations",
  "noIssuesFound": "No Issues found",
  "noOfProjects": "プロジェクト数",
  "noOfScans": "no. of Scans",
  "noPersonalToken": "トークンが見つかりません",
  "noPreference": "指定なし",
  "noProject": "プロジェクトがありません",
  "noProjectUploaded": "プロジェクトがアップロードされていません",
  "noScansFound": "診断が見つかりません",
  "note": "Note",
  "noTeams": "No Teams",
  "noTeamsMessage": "You haven't created any team except <b class='bold'>Default Team</b> which is added to the project by default",
  "notifications": "通知",
  "ok": "OK",
  "operational": "operational",
  "or": "または",
  "orginalRisk": "Original Risk",
  "organization": "組織",
  "organizationMembers": "組織メンバー",
  "organizationNameUpdated": "Successfully updated the organization name",
  "organizationTeamNameUpdated": "Successfully updated the team name",
  "orgMemberInvited": "Invitation email sent!",
  "osVersion": "OSバージョン",
  "overallScanCount": "Overall Scan Count",
  "overrideRisk": "Override Risk",
  "overriddenRisk": "Overridden Risk",
  "owner": "オーナー",
  "owaspDetails": "OWASP DETAILS",
  "owaspMobileCategory": "OWASP MOBILE CATEGORIES",
  "owaspWebCategory": "OWASP WEB CATEGORIES",
  "permissionChanged": "権限が変更されました",
  "package": "パッケージ",
  "packageName": "パッケージ名",
  "pageNotFound": "ページが見つかりません",
  "paid": "有料",
  "password": "パスワード",
  "passwordLengthError": "パスワードは6桁以上でなければなりません",
  "passwordMatchError": "パスワードが一致しません",
  "passwordIsSet": "Password successfully set",
  "passwordIsReset": "パスワードは正常にリセットされました",
  "passwordChanged": "パスワードが変更されました",
  "passwordCopied": "パスワードがコピーされました",
  "tokenCopied": "トークンがコピーされました",
  "trialWillBeConverted": "無料トライアルは有料サブスクリプションに移行されます：",
  "trialWillExpireOn": "トライアル期限： ",
  "pasteYourUrl": "Android / WindowsのストアURLを貼り付けてください",
  "pay": "支払い請求周期",
  "paymentFailed":"PAYMENT FAILED!",
  "paymentError": "Sorry, an error occurred while handling your payment",
  "paymentThank": "THANK YOU!",
  "paymentSuccessful": "Your payment is successful",
  "paymentUpdateFailed": "PAYMENT FAILED TO UPDATE!!!",
  "pdfReport": "Download Report",
  "pending": "ペンディング中",
  "pendingInvitations": "ペンディング中の招待",
  "pendingInvitationsTeam": "Pending Team Invitations",
  "personalAccessTokens": "作成されました ->　作成日",
  "personalAccessTokenCreate": "トークンの生成",
  "personalAccessTokenDesc": "生成したトークンはAppknox APIへのアクセスに使用することができます。",
  "personalTokenKey": "トークン",
  "personalTokenRevoke": "無効にする",
  "personalTokenRevoked": "パーソナルアクセストークンが無効になりました",
  "personalTokenGenerated": "パーソナルアクセストークンが生成されました",
  "phone" : "Phone",
  "plans": "プラン",
  "planName": "プラン名",
  "platform": "Platform",
  "pleaseEnterAllDetails": "プラン名",
  "pleaseEnterAnyNamespace": "名前空間を入力してください",
  "pleaseVisit": "ご案内",
  "pleaseTryAgain": "失敗しました。もう一度お試しください。",
  "pleaseWait": "Please Wait",
  "popOutModal": "全画面",
  "port": "Port",
  "previous": "前",
  "pricing": "価格",
  "proceed": "診断を続行する",
  "production": "プロダクション",
  "project": "プロジェクト",
  "projectCollaboratorAdded": "Added collaborator to project",
  "projectCount": "Projects count",
  "projects": "プロジェクト",
  "projectName": "プロジェクト名",
  "projectRemoved": "プロジェクトが削除されました",
  "projectTeamAdded": "Added team to project",
  "proxyEdit": "Edit in project settings",
  "proxyEnable": "Enable Proxy",
  "proxyWarning": "Please check if you have enabled proxy",
  "proxySettingsTitle": "API Proxy",
  "proxySettingsDescription": "Route dynamic scan and API scan requests via proxy server",
  "proxySettingsRouteVia": "Routing all API requests during dynamic scan & API scan via",
  "proxySettingsSaved": "Proxy settings saved",
  "proxyTurned": "Proxy turned ",
  "purgeAPIheading": "Purge API Analyses",
  "purgeAPIDescription": "Please enter the id of the file you want to purge off API Analyses",
  "quarterly": "３か月",
  "reasonForOverride": "Reason for override",
  "recover": "復元",
  "recoverPassword": "パスワードの復元",
  "register": "登録",
  "registerConfirmation": "Appknoxに登録していただきありがとうございます。提供された電子メールにアクティベーションリンクが送信されました。",
  "invitationRegisterConfirmation": "Thank you for registering with Appknox. Please login to continue.",
  "regulatory": "Regulatory",
  "reject": "Reject",
  "remove": "削除",
  "removeCollaborator": "Remove Collaborator",
  "removeFromTeam": "Remove from team",
  "reportIsGettingGenerated": "レポートが生成されている間お待ちください",
  "reportPassword": "レポートのパスワード",
  "requested": "リクエストしました",
  "requestStatus": "Request",
  "requestToAddNamespace": "新しい名前空間の追加をリクエストしました",
  "rescanInitiated": "再診断を開始しました",
  "resend": "Resend",
  "resendInvitation": "Resend invitation",
  "reset": "リセット",
  "resetPassword": "パスワードのリセット",
  "reportPreference": "Report Preferences",
  "reportScanType":"Choose types of scans to be included in the report",
  "repoIntegrated": "リポジトリの連携に成功しました",
  "repoNotIntegrated": "リポジトリの更新時に問題が発生しました",
  "revokeGithub": "Githubを取り消す",
  "revokeJira": "JIRAを取り消す",
  "risk": "リスク",
  "riskOf": "リスク",
  "riskType": "リスクタイプ",
  "riskAndCommentRequired": "Risk and comment are required fields",
  "role": "役割",
  "save": "Save",
  "saveCredentials": "資格情報の保存",
  "savedPreferences": "設定を保存しました",
  "scan": "診断",
  "scanning": "診断中",
  "scanCompleted": "scan completed",
  "scanDetails": "診断の詳細",
  "scansLeft": "未実施の診断",
  "scanNotCompleted": "scan not completed",
  "scanStatus": "診断のステータス",
  "search": "検索",
  "searchQuery": "パッケージ名で検索",
  "searchMembers": "Search Members",
  "searchProject": "Search for Project",
  "searchTeam": "Search Team",
  "security": "セキュリティ",
  "securityDashboard": "Security Dashboard",
  "selected": "Selected",
  "selectProject": "Select Project",
  "selectThreshold": "Select threshold",
  "showHiddenAnalysis": "Do you want to show the hidden analysis in the Report?",
  "selectAnyTeam": "チームを選択してください",
  "selectedDevice": "選択したデバイスの種類",
  "selectedVersion": "選択したOSのバージョン",
  "selectDevice": "デバイスを選択する",
  "selectLanguage": "以下より言語を選択してください",
  "selectRisk": "Select Risk",
  "selectTeam": "チームを選択してください",
  "selectVulnerabilty": "Please select a vulnerability",
  "settings": "設定",
  "serverReachFailed": "Unable to reach server. Please try after sometime",
  "severityLevel": "重大度レベル",
  "showVulnerability": "脆弱性を表示する",
  "showUntestedAnalysis": "SHOW/HIDE Untested Analysis",
  "signin": "サインイン",
  "sortBy": "並び替え",
  "somethingWentWrong": "Something went wrong, please try again",
  "ssoLogin": "Login using SSO",
  "staging": "ステージング",
  "start": "開始",
  "startDynamicScan": "Start Dynamic Scan",
  "startNewScan": "新しい診断を開始する",
  "started": "開始しました",
  "startingApiScan": "Starting API Scan",
  "startingScan": "診断を開始しています",
  "static": "静的",
  "staticScan": "静的診断",
  "status": "ステータス",
  "stop": "停止中",
  "storage": "Storage",
  "submit": "Submit",
  "submitUrl": "URLを送信",
  "subscription": "サブスクリプション",
  "subscriptionCancelled": "サブスクリプションが正常にキャンセルされました",
  "subscriptionDetails": "サブスクリプションの詳細",
  "subscriptionExpired": "サブスクリプションの期限切れ ",
  "subscriptionWillExpire": "サブスクリプションは失効します",
  "subscriptionWillExpireOn": "サブスクリプションの有効期限：",
  "successfullyAddedAnalysis": "Analysis Added Successfully",
  "successfullyOverridden": "You have successfully overridden this analysis",
  "successfullyPurgeAnalysis": "Successfully Purged the Analysis",
  "successfullyReset": "You have successfully reverted back to orginal risk of this analysis",
  "summary": "サマリー",
  "support": "サポート",
  "sync": "同期する",
  "system": "System",
  "systemStatus": "System Status",
  "tablet": "タブレット",
  "tagAdded": "Tag Added",
  "tagDeleted": "Tag Deleted",
  "team": "チーム",
  "teams": "チーム",
  "teamsAndCollaborators": "Teams & Collaborators",
  "teamCreated": "チームが作成されました",
  "teamDeleted": "削除されました",
  "teamInvitations": "Team Invitations",
  "teamName": "チーム名を入力してください",
  "teamMember": "チームメンバー",
  "teamMemberAdded": "チームメンバーが追加されました",
  "teamMemberInvited": "チームメンバーが招待されました",
  "teamMemberRemoved": "がチームから削除されました",
  "teamProjectAdded": "Project added to team",
  "teamRemoved": "Team removed from accessing this project",
  "testCredentials": "テスト資格情報",
  "topIssues": "Top Issues",
  "title": "Title",
  "totalAppScans": "Total App Scans",
  "totalStaticScan": "Total Static Scans",
  "unavailable": "unavailable",
  "unreachable": "Unreachable",
  "unchanged": "変更なし",
  "undefined": "未定義",
  "unknown": "未知",
  "unpaid": "未払い",
  "untested": "Untested",
  "update": "Update",
  "uploadApp": "アプリのアップロード",
  "uploadCsv": "Upload CSV file",
  "uploading": "アップロード中",
  "uploadFile": "Upload File",
  "uploadNewProject": "新しいプロジェクトをアップロードするには、ここをクリックしてください",
  "urlUpdated": "Successfully updated the url filter",
  "usd": "米ドル",
  "user": "User",
  "userRoleUpdated": "User role changed successfully",
  "userSetting": "ユーザ設定",
  "username": "ユーザ名",
  "version": "バージョン",
  "versionCode": "バージョンコード",
  "vulnerabilities": "脆弱性",
  "vulnerability": "脆弱性",
  "vulnerabilityDetails": "脆弱性の詳細",
  "vulnerabilityPreferences": "Vulnerability Preferences",
  "vulnerabilityPreferencesCondition": "Changes made to vulnerability here will be applied to all future analyses of this project",
  "vulnerabilityPreferencesEdit": "You can edit the vulnerability preferences below",
  "warning": "WARNING",
  "welcome": "ようこそ",
  "worsened": "悪化",
  "year": "年",
  "yearly": "１年",
  "yes": "はい",
  "youCanCreateTeam": "you can create a new team by",
  "youWillBeChargedOn": "次のとおり請求されます：",
  "repo": "Repository",
  "threshold": "Threshold",
  "typeRepoName": "Type Repository name",
  "confirmBox": {
    "removeURL": "URLフィルタから削除してもよろしいですか？",
    "removeProject": "Githubプロジェクトを削除してもよろしいですか？",
    "removeProjectCollaborator": "Are you sure you want to remove this collaborator?",
    "removeProjectTeam": "Are you sure you want to remove this team from project?",
    "removeTeamProject": "Are you sure you want to remove this project from team?",
    "removeJIRA": "JIRAプロジェクトを削除してもよろしいですか？",
    "revokeGithub": "Githubとの連携を解除してもよろしいですか？",
    "revokeJira": "JIRAとの連携を解除してもよろしいですか？",
    "cancelSubsciption": "サブスクリプションをキャンセルしてよろしいですか？",
    "rejectNamespace": "Are you sure you want to delete this namespace?",
    "revokePersonalToken": "パーソナルアクセストークンを削除してよろしいですか？",
    "removeUserRole":	"Are you sure you want to remove this role?",
    "deleteInvitation": "Are you sure you want to delete this invitation?",
    "resendInvitation": "Are you sure you want to resend this invitation?"
  },
  "promptBox": {
    "removeMemberPrompt": {
      "title": "このチームメンバーを削除してもよろしいですか？",
      "description": "削除したいチームメンバーの名前を入力してください"
    },
    "deleteTeamPrompt": {
      "title": "このチームを削除してもよろしいですか？",
      "description": "削除したいチーム名を入力してください"
    }
  },
  "modalCard": {
    "enableMFA": {
      "title": "マルチファクタ認証を有効にする",
      "description": "マルチファクタ認証は、アカウントのセキュリティを更に強化します。 ユーザ名とパスワードに加えて、携帯電話で生成されたコードを入力する必要があります。",
      "setUp": "アプリを使ってセットアップする",
      "downloadAppOTP": "OTPを入手するには、これらのアプリの1つをダウンロードしてください：",
      "extraDescription": "マルチファクタ認証は、アカウントにセキュリティ層を追加します。そのため、パスワードが漏洩したり盗まれたりした場合にも、あなただけがログインすることができます。",
      "scanBarcode": "あなたのアプリでこのバーコードをスキャンする",
      "enterCode": "バーコードをスキャンした後、アプリに表示される6桁のコードを入力してください。"
    },
    "disableMFA": {
      "title": "マルチファクタ認証を無効にする",
      "description": "マルチファクタ認証を無効にすると、アカウントはパスワードでのみ保護されます。",
      "enterOTP": "確認するには、OTPを入力してください"
    },
    "addCollaborator": {
      "title": "Add collaborator to project"
    },
    "addMember": {
      "title": "メンバーを追加"
    },
    "addProject": {
      "title": "Add project to team"
    },
    "addTeam": {
      "title": "Allow teams to access this project"
    },
    "dynamicScan": {
      "title": "Dynamic Scan",
      "warning": "Make sure you perform all the actions in the App, there are high chances that the scanner might mark the risk as passed",
      "deviceRequirements": "App's device requirements",
      "osVersion": "OS version",
      "orAbove": "or above",
      "processorArchitecture": "Processor architecture",
      "deviceTypes": "Device types",
      "runApiScan": "Enable API Capture",
      "apiScanDescription": "During dynamic scan <strong>network traffic made by the app, filtered by the below provided API endpoints</strong> will be recorded. Currently traffic capturing is <strong>not supported for the apps that employ SSL pinning.</strong>",
      "start": "Start Dynamic Scan",
      "statusUpdate": "Dynamic scan status updated",
      "totalScan": "Total Dynamic Scans"
    },
    "apiScan": {
      "title": "API Scan",
      "description": "We will run API scan for web vulnerabilities on the below list of requests captured during the dynamic scan.",
      "warning": "API scan performs multiple automated tests that may result in <strong>denial of service</strong> or <strong>loss of data</strong>. Do not proceed if the app is configured to use a <strong>production API server</strong>.",
      "noDynamicScan": "Please run dynamic scan with API URL filters set. That will capture APIs required for the API scan",
      "start": "Start API Scan",
      "statusUpdate": "Successfully saved the API scan status",
      "totalScan":"Total API Scans"
    },
    "devicePreference": {
      "title": "デバイスの設定変更",
      "selectDevice": "デバイスの選択",
      "note": "changing the device preference won't affect the already running scan."
    },
    "runAPIScan": {
      "title": "API診断の実行",
      "description": "キャプチャしたAPIスキャンを実行する",
      "runScan": "動的＆API診断の実行",
      "noCapturedAPIsData": "Oops, no captured API's found during dynamic scan",
      "capturedWarning": "Run Dynamic Scan first",
    },
    "requestAccess": {
      "title": "Request Access",
      "description": "Trial account does not have access to this feature",
      "button": "Request Access"
    },
    "manual": {
      "title": "手動診断をリクエストする",
      "description": "手動診断をリクエストするには追加情報が必要となります",
      "companyName": "会社名",
      "appName": "アプリ名",
      "appEnv": "アプリケーション環境",
      "minOSVersion": "最小OSバージョン",
      "enterMinOSVersion": "OSの最小バージョンを入力してください",
      "appQuestion1": "一部の機能にアクセスできない、またはアプリケーションに公開されていない機能がある場合は、どうしたらいいですか？",
      "appQuestion1Answer1": "再診断を実施しますので、診断を続行し、診断されなかった機能についてレポートで報告してください。",
      "appQuestion1Answer2": "診断を停止して、サポートに連絡してください。",
      "loginRequired": "あなたのアプリケーションは、ログインするためのアカウントが必要ですか？",
      "enterLoginDetails": "アプリにログインするために必要な資格情報を入力してください<b class = 'bold'>（各役割に4つの資格情報が設定されています）</ b>",
      "addUserRole": "ユーザの役割を追加する",
      "userRoles": "ユーザの役割を追加する",
      "actions": "アクション",
      "poc": "手動診断に関する問い合わせ窓口",
      "vpnRequired": "クライアントからアプリのサーバーにアクセスするにはVPN/プロキシが必要ですか？",
      "enterVPNDetails": "VPNアドレスとポートを入力してください",
      "vpnAddress": "VPNアドレス",
      "vpnPort": "ポート",
      "enterCredentails": "資格情報が必要な場合は、ユーザー名とパスワードを入力してください",
      "additionalComments": "追加コメント",
      "pleaseEnterAllValues": "すべての値を入力してください",
      "roleAdded": "役割が追加されました",
      "pleaseEnterUserRoles": "ユーザの役割を入力してください",
      "pleaseEnterVPNDetails": "VPNの詳細を入力してください",
      "basicAppInfo": "アプリの基本情報",
      "loginDetails": "ログインの詳細",
      "vpnDetails": "VPNの詳細",
      "statusUpdate": "Manual Scan Status Updated",
      "totalScan": "Total Manual Scans"
    },
    "rescan": {
      "title": "Re-scan Static Scan",
      "description": "Do you want to do the static scan again?"
    },
    "copyPassword": {
      "title": "Copy PDF Report Password"
    },
    "orgModal": {
      "title": "Please enter organization name"
    }
  },
  "templates": {
    "apiScanURLFilter": "API診断のURLフィルタ",
    "enterEndpoint": "診断するAPIエンドポイントを入力してください。 api.appknox.com. スキーム（http：// ...）、ポート（：443 / ...）、パス（... / users）は指定しないでください。",
    "enterAPIEndpoint": "Enter host",
    "saveFilter": "フィルタを保存する",
    "addNewUrlFilter": "Add Filter"
  },
  "otherTemplates": {
    "showUntestedAnalysis": "Do you want us to show untested analysis?",
    "specifyTheURL": "Specify host names for filtering network requests during the dynamic scan. If left empty, all requests will be captured",
    "addATeam": "このプロジェクトにコラボレートするチームを追加する",
    "selectGHRepo": "プロジェクトで見つかったリスクに関するIssueを作成するGithub のリポジトリを選択してください",
    "selectJIRAAccount": "プロジェクトで見つかったリスクに関するIssueを作成するJIRAアカウントを選択してください",
    "selectPreferredDevice": "動的診断のデバイスタイプとOSバージョンを選択してください"
  },
  "notFound":{
    "headingOne": "Holy, Carbohydrates!",
    "headingTwo": "Aliens Abducted this page.",
    "headingThree": "Run, Forest! Run!! Run like the wind!!! Run for your life!!! Before the aliens abduct you too.",
    "description": "Oh wait. Dang! This is just a broken page. You scared us for a second! We were about to abandon our offices. Anyway, do not panic. You are just lost."
  },
  "cvssMetricsData":{
    "metricAttackVector": "Attack vector",
    "metricAttackComplexity": "Attack Complexity",
    "metricPrivileges": "Previliges Required",
    "metricInteraction": "User Interaction",
    "metricScope": "Scope",
    "metricConfImpact": "Confidentiality Impact",
    "metricIntegrityImpact": "Interity Impact",
    "metricAvailImpact": "Available Impact",
    "cvssMetricsVector":{
      "NETWORK":"NETWORK",
      "ADJACENT":"ADJACENT",
      "LOCAL":"LOCAL",
      "PHYSICAL":"PHYSICAL",
      "UNKNOWN":"UNKNOWN",
    },
    "cvssMetricsInteraction":{
      "NOT REQUIRED": "NOT REQUIRED",
      "REQUIRED":"REQUIRED",
      "UNKNOWN":"UNKNOWN"
    },
    "cvssMetricsScope":{
      "UNCHANGED":"UNCHANGED",
      "CHANGED": "CHANGED",
      "UNKNOWN":"UNKNOWN"
    },
    "cvssMetricsImpact":{
      "NONE":"NONE",
      "LOW":"LOW",
      "HIGH":"HIGH",
      "UNKNOWN":"UNKNOWN"
    }
  },
};
