...
export default class OffsecScanAdapter extends CommonDRFAdapter {
  ...
  fetchArtifactDownloadUrl(
    ...
    let url = `${this._buildURL(modelName, scanId)}/artifacts/${encodeURIComponent(
      artifactName
    )}/download_url`;

    if (downloadName) {
      url += `?filename=${encodeURIComponent(downloadName)}&download_name=${encodeURIComponent(downloadName)}`;
    }
