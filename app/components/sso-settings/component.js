...
export default Component.extend({
  ...
  spConfigIsManual: computed('spConfig', function () {
    console.log('spConfig', this.get('spConfig'));
  ...
  SPMetadata: task(function* () {
    const spMetadata = yield this.get('ajax').request(
      ENV.endpoints.saml2SPMetadata
    );
  ...
  toggleSSOEnableSucceeded: on(
    'toggleSSOEnable:succeeded',
  ...
  toggleSSOEnforceSucceeded: on(
    'toggleSSOEnforce:succeeded',
    function (taskInstance) {
      const [toggleValue] = taskInstance.args;
      if (toggleValue) {
