...
const OIDCValidations = {
  ...
  discovery_url: [validatePresence(true), validateFormat({ type: 'url' })],
};
...
export default class SsoSettingsOidcComponent extends Component<SsoSettingsOidcSignature> {
  ...
  get hasProviders() {
    return this.args.providers.length > 0;
  }

  get discoveryUrl() {
    return this.args.providers[0]?.get('discoveryUrl') || '';
  }
  ...
  deleteProvider = task({ drop: true }, async () => {
    const provider = this.args.providers[0];
