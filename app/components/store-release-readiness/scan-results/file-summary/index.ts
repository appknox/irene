...
import './index.scss';
import { StoreReleaseReadinessCardData } from '../../release-card';
...
export default class StoreReleaseReadinessScanResultsFileSummaryComponent extends Component<StoreReleaseReadinessScanResultsFileSummarySignature> {
  ...
  get appLogoSrc(): string {
    return this.args.item.iconUrl?.trim();
  }
  ...
  get showReportButton() {
    return (
      !this.loading &&
      (this.args.item.analysisStatus === 'completed' ||
        this.args.item.analysisStatus === 'partial')
    );
  }
  ...
  /** Spinner only while the scan is actively running (not partial / not started / failed). */
