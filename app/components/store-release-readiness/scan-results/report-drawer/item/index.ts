...
export default class StoreReleaseReadinessReportDrawerItemComponent extends Component<StoreReleaseReadinessReportDrawerItemSignature> {
  ...
  get isReportGenerating() {
    return (
      this.report?.pdfStatus === ENUMS.STORE_RELEASE_REPORT_STATUS.STARTED ||
      this.report?.pdfStatus === ENUMS.STORE_RELEASE_REPORT_STATUS.IN_PROGRESS
    );
  }

  get isReportGenerated() {
    return (
      this.report?.pdfStatus === ENUMS.STORE_RELEASE_REPORT_STATUS.COMPLETED
    );
  }
  ...
  handleDownloadReport = task(async () => {
    ...
    try {
      const data = await waitForPromise(this.report.downloadReport());
  ...
  handleGenerateReport = task(async () => {
    ...
    try {
      await waitForPromise(this.report.generateReport());
      await waitForPromise(this.report.reload());
    } catch (e) {
      this.notify.error(parseError(e, this.tPleaseTryAgain));
    }
  });
