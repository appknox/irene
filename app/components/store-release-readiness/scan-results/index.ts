...
export default class StoreReleaseReadinessScanResultsComponent extends Component<StoreReleaseReadinessScanResultsSignature> {
  ...
  /** Resolved once per `loadScan`; version fields on card are otherwise derived from `scan`. */
  @tracked scanFile: FileModel | null = null;

  @tracked assessmentPolicyFilter: AssessmentPolicyFilter = 'all';

  @tracked selectedCategoryLabels: string[] = [];

  /** Avoid duplicate `loadScan` when `did-update` fires with the same release id. */
  #loadedReleaseId: string | null = null;
  ...
  constructor(
    ...
    this.#loadedReleaseId = args.releaseId;
    this.loadScan.perform(args.releaseId);
  }

  /**
   * Derived from the live `scan` model so websocket / store pushes update FileSummary & Details
   * the same way `isScanNotStarted` and `assessmentRows` already do.
   */
  get cardData(): StoreReleaseReadinessCardData | null {
    if (!this.scan) {
      return null;
    }

    return cardDataFromScan(this.scan, this.scanFile);
  }
  ...
  @action
  onReleaseIdChanged() {
    if (this.args.releaseId === this.#loadedReleaseId) {
      return;
    }

    this.#loadedReleaseId = this.args.releaseId;
    this.loadScan.perform(this.args.releaseId);
  }
  ...
  get isScanFailed(): boolean {
    return this.scan?.status === ENUMS.STORE_RELEASE_SCAN_STATUS.FAILED;
  }

  get isScanNotStarted(): boolean {
    return this.scan?.status === ENUMS.STORE_RELEASE_SCAN_STATUS.NOT_STARTED;
  }
