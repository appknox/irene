...
export default class FileDetailsDynamicScanDrawerDevicePrefTableComponent extends Component<FileDetailsDynamicScanDrawerDevicePrefTableSignature> {
  ...
  constructor(
    owner: unknown,
    args: FileDetailsDynamicScanDrawerDevicePrefTableComponent['args']
  ) {
    super(owner, args);
  }
  ...
  @action setDevicePrefFilter(
    opt: keyof typeof AvailableManualDeviceModelKeyMap
  ) {
  ...
  filterAvailableDevices = task(
    async (filter: keyof typeof AvailableManualDeviceModelKeyMap) => {
      ...
      if (filter !== 'All Available Device') {
        this.filteredManualDevices = this.allAvailableManualDevices.filter(
          (dev) => dev[modelFilterKey]
        );
      } else {
        this.filteredManualDevices = this.allAvailableManualDevices;
      }
    }
  );
}
