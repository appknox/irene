...
export default class FileDetailsDynamicScanDrawerOldComponent extends Component<FileDetailsDynamicScanDrawerOldSignature> {
  ...
  saveDevicePrefToLocalStorage = task(async () => {
    ...
    if (this.deviceTypeIsAny) {
      const randomDeviceType = this.pickRandomItemFromList(
        this.devicePrefContext.deviceTypes.filter(
          (d) => d.value !== ENUMS.DEVICE_TYPE.NO_PREFERENCE
        )
      );

      modifiedDevicePref.device_type = randomDeviceType?.value;
    }
    ...
    this.window.localStorage.setItem(
      ...
      JSON.stringify(this.currentDevicePref)
    );
