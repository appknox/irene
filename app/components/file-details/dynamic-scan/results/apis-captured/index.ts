...
export default class FileDetailsDynamicScanResultsApisCaptured extends Component<FileDetailsDynamicScanResultsApisCapturedSignature> {
  ...
  get capisHeaderText() {
    return this.hasNoCumulativeCapturedApis
      ? 'No Unique APIs Requests have been captured so far'
      : `${this.fileCumulativeCapturedApisCount} Unique API Request(s) have been captured so far`;
  }

  get capisDescriptionText() {
    return this.hasNoCumulativeCapturedApis
      ? 'Perform DAST scan by enabling API Capture to get the data.'
      : 'The data in the file contains cumulative data of all the DAST scan done so far.';
  }

  @action
  handleDownloadCapturedApis() {
    if (!this.fileCumulativeCapturedApisCount) {
      this.notify.error('No APIs have been captured!');
