...
export default class FileDetailsDynamicScanResultsCapisReportDrawerComponent extends Component<FileDetailsDynamicScanResultsCapisReportDrawerSignature> {
  ...
  get cannotGenerateReportFiles() {
    return this.intl.t('apiScanModule.cannotGenerateReportFiles');
  }
  ...
  get enableGenerateBtn() {
    return !this.file.isDynamicScanRunning && !this.latestReportIsGenerating;
  }
  ...
  generateReport = task(async () => {
    // If dynamic scan is running, do not generate report
    if (this.file.isDynamicScanRunning) {
      this.notify.error(this.cannotGenerateReportFiles);
