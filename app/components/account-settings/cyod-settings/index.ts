...
export default class AccountSettingsCyodSettingsComponent extends Component<AccountSettingsCyodSettingsSignature> {
  ...
  get mercerMacDownloadUrl() {
    return this.buildMercerDownloadUrl('Mercer-x86_64.dmg');
  }

  get mercerWindowsDownloadUrl() {
    return this.buildMercerDownloadUrl('Mercer-windows-x64.exe');
  }

  get mercerLinuxDownloadUrl() {
    return this.buildMercerDownloadUrl('mercer.deb');
  }

  buildMercerDownloadUrl(fileName: string) {
    return `${ENV.mercerDownloadUrl.replace(/\/+$/, '')}/${fileName}`;
  }
