...
export default class DsPreferenceProviderComponent extends Component<DsPreferenceProviderSignature> {
  ...
  get selectedAvailableManualDevice() {
    const id = this.dsManualDevicePreference?.dsManualDeviceIdentifier;

    return this.availableDevicesResponse
      ?.slice()
      .find((d) => d.deviceIdentifier === id);
  }
