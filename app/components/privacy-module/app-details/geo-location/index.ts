...
// --- TypeScript declaration for testing ---
declare global {
  interface Window {
    QUnit?: unknown;
  }
...
export default class PrivacyModuleAppDetailsGeoLocationComponent extends Component<PrivacyModuleAppDetailsGeoLocationSignature> {
  ...
  constructor(
    ...
    this.privacyModule.fetchGeoLocationData.perform(this.fileId, true);
  }
  ...
  async setupChart(element: HTMLElement): Promise<void> {
    ...
    if (window.QUnit) {
      (element as any).__chart__ = this.chart;
      (element as any).__component__ = this;
    }
      ...
      setTimeout(() => {
        ...
        if (btn) {
          btn.onclick = () => {
            ...
            const seriesArray = option['series'] as { data: unknown[] }[];
