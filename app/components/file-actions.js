...
export default Component.extend({
  ...
  vulnerabilities: computed('file.project.id', 'store', function () {
    ...
    return store.query("security/vulnerability", {
        projectId,
        limit: 0
      })
      .then(
        data => store.query("security/vulnerability", {
          projectId,
  ...
  confirmPurge: task(function* () {
    ...
    return yield this.get("ajax").post(url, {
  ...
  toggleAddAnalysisModal: task(function* () {
    yield this.set("showAddAnalysisModal", !this.get('showAddAnalysisModal'));
  }),
