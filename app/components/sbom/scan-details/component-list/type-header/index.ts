...
export default class FileDetailsDynamicScanResultsCoverageTableComponentTypeHeaderComponent extends Component<FileDetailsDynamicScanResultsCoverageTableComponentTypeHeaderSignature> {
  ...
  constructor(
    owner: unknown,
    args: FileDetailsDynamicScanResultsCoverageTableComponentTypeHeaderSignature['Args']
  ) {
    super(owner, args);

    this.selectedComponentType = args.selectedComponentType;
  }
  ...
  @action selectComponentType(value: number) {
    this.selectedComponentType = value;
    this.anchorRef = null;
