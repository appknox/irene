...
export default class AiBomComponentListComponent extends Component<AiBomComponentListSignature> {
  ...
  get summaryItems(): SbomScanDetailsSummaryBarItem[] {
    ...
    const typeItems: SbomScanDetailsSummaryBarItem[] = [
      ...
      {
        iconName: 'solar:link-linear' as const,
        label: this.intl.t('sbomModule.aiTypeLabel.runtimeModelReference'),
        value: byType['runtime_reference'] ?? 0,
      },
      {
  ...
  get columns() {
    return [
      ...
      {
        name: this.intl.t('sbomModule.purpose'),
        valuePath: 'aiPurposeDisplay',
      },
    ];
  }
