...
const GithubProjectComponent = Component.extend({
  ...
  setCurrentGithubRepoErrored: on(
    'setCurrentGithubRepo:errored',
    function (_, err) {
      if (
        err.errors[0].detail &&
  ...
  setCurrentGithubRepoSucceeded: on(
    'setCurrentGithubRepo:succeeded',
    function (instance) {
  ...
  getGithubRepos: task(function* () {
    let url = `/api/organizations/${this.get(
  ...
  selectProject: task(function* () {
    ...
    if (githubRepo) {
      ...
      githubRepo = this.get('store').createRecord('github-repo', {
