...
export default class StoreknoxInventoryDetailsAppDetailsVaResultsComponent extends Component<StoreknoxInventoryDetailsAppDetailsVaResultsSignature> {
  get coreProjectLatestVersion() {
    return this.args.app?.coreProject?.latestVersion;
  }

  get vaResults() {
    return {
      critical: this.coreProjectLatestVersion?.riskCountCritical,
      high: this.coreProjectLatestVersion?.riskCountHigh,
      medium: this.coreProjectLatestVersion?.riskCountMedium,
      low: this.coreProjectLatestVersion?.riskCountLow,
      passed: this.coreProjectLatestVersion?.riskCountPassed,
      unknown: this.coreProjectLatestVersion?.riskCountUnknown,
    };
  }

  get vaResultsCategories() {
    return Object.keys(this.vaResults) as Array<
      'medium' | 'critical' | 'high' | 'low' | 'passed' | 'unknown'
    >;
  }
