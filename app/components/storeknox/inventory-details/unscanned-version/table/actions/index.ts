...
export default class StoreknoxInventoryDetailsUnscannedVersionTableActionsComponent extends Component<StoreknoxInventoryDetailsUnscannedVersionTableActionsSignature> {
  ...
  get uploadIncomplete() {
    return (
      ...
      (!this.userCanAccessSubmission && this.uploadIsInProgressForNonInitiator)
    );
  }

  get showInitiateAppUploadButton() {
    return this.canInitiateUpload;
  }
  ...
  get uploadHeaderMsg() {
    if (this.userCanAccessSubmission) {
      ...
      if (!this.canInitiateUpload) {
        return this.isOwner
          ? 'Upload Pending Completion'
          : 'Upload to Appknox Already Initiated';
      }
    } else {
      if (this.canInitiateUpload) {
      ...
      if (!this.canInitiateUpload && this.isOwner) {
        return 'Upload Pending Completion';
      }
    }

    return 'Upload to Appknox Already Initiated';
  }

  get uploadSubTextMsg() {
    if (this.userCanAccessSubmission) {
      if (this.fileIsBeingAnalyzed) {
        return 'This app upload has been completed. Kindly give a minute to get VA results.';
      }
    ...
    return this.submissionErrorMsg
      ...
      : 'Upload has been initiated and is still in progress please check back later.';
  }

  get isIOSApp() {
    return this.skApp.get('appMetadata')?.platform === ENUMS.PLATFORM.IOS;
  }
  ...
  initiateUpload = task(async () => {
    ...
    try {
      ...
      this.notify.error('You cannot initiate an upload for this version.');
    } catch (error) {
