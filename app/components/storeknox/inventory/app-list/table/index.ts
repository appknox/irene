...
import { StoreknoxCommonTableColumnsData } from 'irene/components/storeknox/table-columns';
...
export default class StoreknoxInventoryAppListTableComponent extends Component<StoreknoxInventoryAppListTableSignature> {
  ...
  @tracked showMaxDisAppsSelectMsgModal = false;
  ...
  get showPagination() {
    return !this.isFetchingTableData && this.totalCount > this.limit;
  }
  ...
  @action selectDisabledAppRow(ulid: string, value: boolean) {
    if (this.selectedDisabledAppIds.length === 5 && value) {
      this.showMaxDisAppsSelectMsgModal = true;
  ...
  @action closeMaxDisAppsSelectMsgModal() {
    this.showMaxDisAppsSelectMsgModal = false;
  }
