/* eslint-disable ember/no-array-prototype-extensions, ember/no-classic-components, ember/no-classic-classes, ember/require-tagless-components, ember/avoid-leaking-state-in-ember-objects, ember/no-get, ember/require-return-from-computed, ember/no-actions-hash */
...
export default Component.extend({
  ...
  allFindings: computed(
    'addedFindings',
    'analysisDetails.findings',
    'findingId',
    function () {
      let findingId = this.get('findingId');
      const findings =
        this.get('addedFindings') || this.get('analysisDetails.findings');
      if (findings) {
  ...
  confirmCallback(key) {
    ...
    if (key === 'passed') {
      ...
      this.set(
        'analysisDetails.attackComplexity',
        ENUMS.ATTACK_COMPLEXITY.HIGH
      );
      this.set(
        'analysisDetails.privilegesRequired',
        ENUMS.PRIVILEGES_REQUIRED.HIGH
      );
      this.set(
        'analysisDetails.userInteraction',
        ENUMS.USER_INTERACTION.REQUIRED
      );
      this.set('analysisDetails.scope', ENUMS.SCOPE.UNCHANGED);
      this.set(
        'analysisDetails.confidentialityImpact',
        ENUMS.CONFIDENTIALITY_IMPACT.NONE
      );
      this.set('analysisDetails.integrityImpact', ENUMS.INTEGRITY_IMPACT.NONE);
      this.set(
        'analysisDetails.availabilityImpact',
        ENUMS.AVAILABILITY_IMPACT.NONE
      );
      this.set(
        'analysisDetails.cvssVector',
  ...
  isEmptyCvssVector() {
    if (
      ...
      this.get('analysisDetails.attackComplexity') ==
        ENUMS.ATTACK_COMPLEXITY.UNKNOWN &&
      this.get('analysisDetails.privilegesRequired') ==
        ENUMS.PRIVILEGES_REQUIRED.UNKNOWN &&
      this.get('analysisDetails.userInteraction') ==
        ENUMS.USER_INTERACTION.UNKNOWN &&
      this.get('analysisDetails.scope') == ENUMS.SCOPE.UNKNOWN &&
      this.get('analysisDetails.confidentialityImpact') ==
        ENUMS.CONFIDENTIALITY_IMPACT.UNKNOWN &&
      this.get('analysisDetails.integrityImpact') ==
        ENUMS.INTEGRITY_IMPACT.UNKNOWN &&
      this.get('analysisDetails.availabilityImpact') ==
  ...
  isValidCvssVector() {
    ...
    if (
      ENUMS.ATTACK_VECTOR.BASE_VALUES.includes(
        this.get('analysisDetails.attackVector')
      ) &&
  ...
  updateCVSSScore() {
    ...
    if (this.isValidCvssVector()) {
      ...
      const confidentialityImpact = this.get(
        'analysisDetails.confidentialityImpact'
      );
    ...
    if (param === 'back') {
      yield getOwner(this)
        ...
        .transitionTo(
          ...
          this.get('analysisDetails.file.id')
        );
    }
  }).evented(),
  ...
  updateAnalysis: task(function* () {
    ...
    const overriddenRiskComment = this.get(
      'analysisDetails.overriddenRiskComment'
    );
    const overriddenRiskToProfile = this.get(
      'analysisDetails.overriddenRiskToProfile'
    );
    const attackVector = this.get('analysisDetails.attackVector');
    const attackComplexity = this.get('analysisDetails.attackComplexity');
    const privilegesRequired = this.get('analysisDetails.privilegesRequired');
  ...
  uploadFile: task(function* (file) {
    ...
    const analysisid = this.get('analysis.analysisid');
    try {
      var fileData = yield this.get('ajax').post(ENV.endpoints.uploadFile, {
        namespace: 'api/hudson-api',
        data,
      });
      ...
      yield this.get('ajax').post(ENV.endpoints.uploadedAttachment, {
        namespace: 'api/hudson-api',
      ...
      const analysisObj = yield this.get('store').findRecord(
        'security/analysis',
        this.get('analysis.analysisid')
      );
  ...
  actions: {
    ...
    addFinding() {
      ...
      if (isEmpty(findingDescription)) {
        return this.get('notify').error('Please fill the description');
      }
      let findingId = this.get('findingId');
      findingId = findingId + 1;
      const findings = this.get('analysisDetails.findings');
    ...
    downloadAttachment(id) {
      ...
      return this.get('ajax')
        .request(url, { namespace: 'api/hudson-api' })
        .then(
          (data) => {
