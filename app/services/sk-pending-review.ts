...
export default class SkPendingReviewService extends Service {
  ...
  approveRejectApp = task(
    async (id: string, approve: boolean, multiple?: boolean) => {
      ...
      let selectedAppValue: SkAppModel | null = null;

      selectedAppValue = this.skPendingReviewData?.toArray()[index] ?? null;
      ...
      try {
        ...
        if (approve) {
          this.notify.success('App has been moved to App Inventory');
        } else {
          this.notify.success('App has been rejected');
        }
      } catch (error) {
        console.log(error);
      }
  ...
  selectRow(id: string, value: boolean, force?: boolean) {
    ...
    let selectedResult: SkAppModel | null = null;

    selectedResult = this.skPendingReviewData?.toArray()[index] ?? null;
  ...
  selectAllRow(value: boolean) {
    ...
    for (const record of this.skPendingReviewData?.toArray() || []) {
  ...
  checkIfAllSelected() {
    if (
      this.selectedApps.length === this.skPendingReviewData?.toArray().length
    ) {
