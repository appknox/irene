...
import RateLimitManager, { type RateLimitState } from 'irene/utils/rate-limit';
...
export default class UploadAppService extends Service {
  ...
  // Rate limit
  @tracked rateLimitState: RateLimitState = RateLimitManager.createState();
  ...
  get rateLimitFormattedTime() {
    return RateLimitManager.formatTime(this.rateLimitState.remainingTime);
  }

  showAndStartRateLimitErrorCountdown(retryTime: number) {
    this.showRateLimitError = true;

    const newState = RateLimitManager.startCountdown(
      this,
      this.rateLimitState,
      retryTime,
      {
  ...
  clearRateLimitError() {
    ...
    const clearedState = RateLimitManager.clearCountdown(
      this,
      this.rateLimitState
    );
