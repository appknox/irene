...
const POLL_TIMEOUT = 600000; //10 minutes timeout is a reasonable upper limit for file processing and report generation.

async function globalSetup() {
  /**
  0. Clean allure-results
  Before we start the setup, we want to clean up any existing allure-results from previous test runs.
  This ensures that our test reports will only include data from the current test run and prevents confusion caused by stale results.
  */
  const allureResultsDir = path.resolve(__dirname, '../allure-results');
  if (fs.existsSync(allureResultsDir)) {
    fs.rmSync(allureResultsDir, { recursive: true, force: true });
    console.log('[Setup] allure-results cleaned');
  } else {
    console.log('[Setup] allure-results not found — nothing to clean');
  }

  /**
  1. API setup
  we initialize our API request wrapper, which will handle making authenticated API requests using the tokens obtained from the UI login.
  ...
  */
  ...
  /**
  3. Get org features
  We also want to check which features are enabled for our organization, as this can impact which tests we can run and how we should run them. 
  By hitting the organization details endpoint with our orgId, we can get the list of features that are enabled for our org and save that information in our state file. 
  This way, our tests can conditionally skip or modify their behavior based on whether certain features are available in the org.
   */
  const orgFeatures = orgBody.results[0].features;
 ...
 */
  ...
  */
  ...
  if (orgFeatures.privacy) {
    ...
    console.log('[Setup] Privacy Shield disabled — skipping');
  }
  // 12. Wait for SBOM scan to complete for our uploaded file

  let sbFileId: number = 0;
  let sbReportId: number = 0;
  ...
  if (orgFeatures.sbom) {
    ...
    sbReportId = sbReportsBody.results[0].id as number;
    console.log(`[Setup] sbReportId: ${sbReportId}`);
  } else {
    console.log('[Setup] SBOM disabled — skipping');
  }
  ...
  */
  ...
  const state = {
    ...
    features: {
      ...
      manualscan: orgFeatures.manualscan as boolean,
      dynamicscan_automation: orgFeatures.dynamicscan_automation as boolean,
      upload_via_url: orgFeatures.upload_via_url as boolean,
    },
  };
