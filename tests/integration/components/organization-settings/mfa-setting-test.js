...
module(
  'Integration | Component | organization-setting/mfa-setting',
  function (hooks) {
    ...
    test('it render nothing, if user is not owner', async function (assert) {
      await render(hbs`<OrganizationSettings::MfaSetting />`);
      assert.dom(`[data-test-mfa-setting]`).doesNotExist();
    });

    test('it renders basic elements for owner', async function (assert) {
      ...
      await render(
        hbs`<OrganizationSettings::MfaSetting
      ...
      assert
        .dom(`[data-test-mfa-setting-title]`)
        .hasText(`t:multiFactorAuth:()`);
      ...
      assert.dom(`[data-test-mfa-setting-warning-note]`).hasClass('alert');
      assert
