...
module('Integration | Component | dynamic-scan/modal', function (hooks) {
  ...
  test.each(
    ...
    async function (assert, { isAutomated }) {
      ...
      assert
        .dom('[data-test-dynamicScanModal-drawerContainer-title]')
        .exists()
        .hasText(
          isAutomated
      ...
      assert
        .dom('[data-test-dynamicScanModal-drawerContainer-closeBtn]')
        .exists();
      ...
      if (!isAutomated) {
        assert.dom('[data-test-dynamicScanModal-manualDast-header]').exists();

        assert
          .dom('[data-test-dynamicScanModal-manualDast-modalBodyWrapper]')
          .exists();
        ...
        assert
          .dom('[data-test-dynamicScanModal-manualDast-headerOSInfoDesc]')
          .exists()
          .containsText('t:modalCard.dynamicScan.osVersion:()');

        assert
          .dom('[data-test-dynamicScanModal-manualDast-headerOSInfoValue]')
          .exists()
          .containsText('t:android:()')
          .containsText('t:modalCard.dynamicScan.orAbove:()')
          .containsText(this.profile.dsAutomatedPlatformVersionMin);

        assert
          .dom('[data-test-dynamicScanModal-manualDast-devicePrefHeaderDesc]')
          .exists()
          .containsText('t:devicePreferences:()');

        assert
          .dom('[data-test-dynamicScanModal-manualDast-devicePrefSelect]')
          .exists();
        ...
        assert
          .dom('[data-test-dynamicScanModal-manualDast-devicePrefTable-root]')
          .doesNotExist();

        assert.dom('[data-test-fileDetails-proxySettings-container]').exists();

        assert
          .dom('[data-test-dynamicScanModal-manualDast-enableAPICapture]')
          .exists()
        ...
        assert
          .dom('[data-test-dynamicScanModal-manualDast-apiFilter-title]')
  ...
  test('manual DAST: it selects a device preference', async function (assert) {
    ...
    this.server.put('/profiles/:id/ds_manual_device_preference', (_, req) => {
    ...
    this.server.get('/profiles/:id/ds_manual_device_preference', () => {
      return {
        ds_manual_device_selection: ENUMS.DS_MANUAL_DEVICE_SELECTION.ANY_DEVICE,
    ...
    assert
      .dom('[data-test-dynamicScanModal-manualDast-devicePrefTableHeaderTitle]')
      .exists()
    ...
    const deviceElemList = findAll(
      '[data-test-dynnamicScanModal-devicePrefTable-row]'
    );
    ...
    const deviceSelectRadioElement =
      '[data-test-dynnamicScanModal-devicePrefTable-deviceSelectRadioInput]';
    ...
    for (let idx = 0; idx < deviceElemList.length; idx++) {
      ...
      Object.keys(deviceCapabilitiesMap).forEach((key) => {
        if (deviceModel[key]) {
          ...
          assert
            .dom(
              `[data-test-dynnamicScanModal-devicePrefTable-capabilityId='${capabilityLabel}']`,
              deviceElem
            )
    ...
    async function (assert, { isAutomated, enableApiCapture }) {
      ...
      this.server.get('/profiles/:id/ds_manual_device_preference', () => {
        return {
          ds_manual_device_selection:
            ENUMS.DS_MANUAL_DEVICE_SELECTION.ANY_DEVICE,
          ds_manual_device_identifier: null,
        };
      });
      ...
      this.server.post('/v2/files/:id/dynamicscans', (_, req) => {
        ...
        assert.strictEqual(reqBody.mode, isAutomated ? 'Automated' : 'Manual');
     ...
     `);
      ...
      assert.dom('[data-test-dynamicScanModal-startBtn]').exists().isDisabled();
      ...
      assert.dom('[data-test-dynamicScanModal-startBtn]').isNotDisabled();
