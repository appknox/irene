...
module(
  ...
  function (hooks) {
    ...
    test('it derives signal result label from numeric aiScore for Brand Identity signals', async function (assert) {
      ...
      const brandGroup = find(
        '[data-test-storeknoxFakeAppsDetails-brandGroup]'
      );
      ...
      // Sorted by score descending: SemanticSimilarity(0.9)=Strong Match, Logo(0.7)=Partial Match, Package(0.3)=Low Match
      assert
      ...
      assert.dom(resultElements?.[2]).hasText(t('storeknox.fakeApps.lowMatch'));
    });
    ...
    test('it uses risk labels for Security Risk Assessment signals', async function (assert) {
      ...
      const securityGroup = find(
        '[data-test-storeknoxFakeAppsDetails-securityGroup]'
      );
      ...
      // ManifestFlagsRule=0.5 → LOW → "Low Risk"
      assert.dom(resultElements?.[1]).hasText(t('storeknox.fakeApps.lowRisk'));
    });
    ...
    test.each(
      ...
      [
        ...
        { level: 'LOW', expectedKey: 'storeknox.fakeApps.lowRisk' },
      ],
    ...
    test('clicking a signal row expand button opens the drawer with the section title and signal details', async function (assert) {
      ...
      const firstExpandBtn = find(
        '[data-test-storeknoxFakeAppsDetails-brandGroup] [data-test-storeknoxFakeAppsFindingsSignalRow-expandBtn]'
      );
