...
const menuItems = ({ analytics, appMonitoring, market, billing, partner }) =>
  [
    ...
    partner && { label: 't:clients:()', icon: 'groups-2', hasBadge: true },
  ].filter(Boolean);
...
module('Integration | Component | home-page/org-side-menu', function (hooks) {
  ...
  test.each(
    ...
    async function (
      assert,
      { owner, admin, billing, partner, market, appMonitoring }
    ) {
      ...
      await render(
        hbs`<HomePage::OrgSideMenu @organization={{this.organization}} />`
      );
  ...
  test('test submission-list triggers based on realtime submission counter', async function (assert) {
    ...
    assert.dom('[data-test-submission-status-detail]').hasClass('is-progress');
