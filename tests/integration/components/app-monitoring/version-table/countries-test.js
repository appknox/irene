...
module(
  ...
  function (hooks) {
    ...
    hooks.beforeEach(async function () {
      ...
      // AmAppStoreInstances
      const amAppStoreInstances = [1, 2, 3, 4].map((id) => {
    ...
    test('it renders loading state and the correct countries code', async function (assert) {
      this.server.get(
      ...
      assert
        ...
        .exists()
        .containsText(
          this.amAppStoreInstances
            .map((record) => record.get('countryCode'))
            .join(', ')
        );
    });
