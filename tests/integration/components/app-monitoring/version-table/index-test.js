  ...
  function (hooks) {
    ...
    hooks.beforeEach(async function () {
      ...
      const amAppVersions = files.map((file, idx) => {
        const amAppVersion = this.server.create('am-app-version', {
          latest_file: faker.random.arrayElement([file.id, null]),
          am_app: amApp.id,
        });
        ...
        return this.store.push(normalizedAmAppVersion);
      });
    ...
    test.each(
      ...
      async function (assert, isHistoryTable) {
        ...
        await triggerEvent(
          versionTableHeaders[tableHeaderTitles.length - 1].querySelector(
        ...
        const firstRowVersionLatestFile =
          this.amAppVersions[0].get('latestFile');
        ...
        if (isHistoryTable) {
          assert
            ...
            .exists()
            .containsText(
              dayjs(firstRowVersion.get('createdOn')).format('Do MMM YYYY')
            );
        } else {
          assert
            .dom('[data-test-amVersionTable-dateFound]', firstRow)
        ...
        assert
          ...
          .exists()
          .containsText(firstRowVersion.get('displayVersion'));
        ...
        const firstRowIsScanned = firstRowVersion.get('latestFile').get('id');
        ...
        const transitionedFromUnscannedToScanned =
          !firstRowVersionLatestFile?.get('createdOn')
            ? false
            : dayjs(firstRowVersionLatestFile?.get('createdOn')).isAfter(
                firstRowVersion.get('createdOn')
              );
        ...
        if (firstRowIsScanned && transitionedFromUnscannedToScanned) {
          ...
          assert
            ...
            .containsText(firstRowVersionLatestFile?.get('id'));
        }
