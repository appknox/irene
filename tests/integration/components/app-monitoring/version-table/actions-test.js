...
module(
  ...
  function (hooks) {
    ...
    test.each(
      'it initiates app upload and shows the correct upload statuses',
      [
    ...
    test('it triggers reload on app upload error', async function (assert) {
      ...
      this.server.post('/v2/am_app_versions/:id/initiate_upload', () => {
        if (this.returnCompletedSubmission) {
          return { id: 2, status };
        }

        this.set('returnCompletedSubmission', true);
