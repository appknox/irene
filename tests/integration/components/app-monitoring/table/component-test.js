...
module('Integration | Component | app-monitoring/table', function (hooks) {
  ...
  test('it should show app drawer when a row item is clicked', async function (assert) {
    this.set('settings', {
      id: 1,
      enabled: true,
    });
    ...
    assert
      ...
      .doesNotHaveClass(styles['open']);
    await click(`[data-test-am-table-row-id="0"]`);
    assert.dom(`[data-test-am-drawer-container]`).hasClass(styles['open']);
  });
});
