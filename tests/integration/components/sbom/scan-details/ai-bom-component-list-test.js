...
module(
  ...
  function (hooks) {
    ...
    test('it includes runtime model reference in the summary bar, both as its own chip and in the total', async function (assert) {
      // Regression guard: the summary bar's category chips used to be a
      // fixed list that didn't know about 'runtime_reference', so the
      // Total (which sums every category the backend returns) could show a
      // higher count than the sum of the visible chips.
      this.server.createList('sbom-component', 2, {
        ai_artifact_class: 'runtime_reference',
      ...
      // The container renders immediately, before the async ai_summary fetch
      // resolves -- wait for the chip itself, which only appears once
      // aiSummaryData has actually loaded and summaryItems recomputes.
      await waitFor(
        `[data-test-sbomScanDetails-summaryBar-item="${t(
          'sbomModule.aiTypeLabel.runtimeModelReference'
        )}"]`
      );
      ...
      assert
        .dom(
          `[data-test-sbomScanDetails-summaryBar-item="${t(
            'sbomModule.aiTypeLabel.runtimeModelReference'
          )}"] [data-test-sbomScanDetails-summaryBar-value]`
        )
        .hasText('2', 'runtime model reference chip shows its own count');

      assert
        .dom(
          `[data-test-sbomScanDetails-summaryBar-item="${t(
            'sbomModule.totalComponents'
          )}"] [data-test-sbomScanDetails-summaryBar-value]`
        )
        .hasText(
          '2',
          'total matches the sum including runtime model reference'
        );
    });

    test('it hides the runtime model reference chip when its count is zero', async function (assert) {
      ...
      assert
        ...
        .doesNotExist('no "X - 0" noise for a category with zero components');
    });
