...
module('Integration | Component | sbom/scan-report-drawer', function (hooks) {
  ...
  test('test ai bom pdf report is backed by its own report record', async function (assert) {
    this.server.get('/v2/sb_files/:id/sb_reports', (schema) => {
      ...
      const aiBomReport = this.server.create('sbom-report', {
        ...
        report_password: 'AIBOMPASSWORD1', //NOSONAR
      });
    ...
    assert
      ...
      .hasText(t('reportPasswordDetail', { password: 'AIBOMPASSWORD1' })); //NOSONAR
  ...
  test('it hides the ai bom report item for scans that predate ai bom detection', async function (assert) {
    this.server.get('/v2/sb_files/:id/sb_reports', (schema) => {
      const results = schema.sbomReports.all().models;
    ...
    this.server.get(
      ...
      () => {
        return { total: 0, by_type: {}, aibom_supported: false };
      }
    );
    ...
    assert
      .dom('[data-test-sbomReportList-reportPrimaryText]')
  ...
  test('it still shows the ai bom report item for a pre ai bom scan that already has ai components', async function (assert) {
    ...
    // matching ai-bom-component-list's showAiBomNewFeaturePrompt rule.
    this.server.get('/v2/sb_files/:id/sb_reports', (schema) => {
      const results = schema.sbomReports.all().models;
    ...
    this.server.get(
      `/v2/sb_files/${this.sbomFile.id}/sb_file_components/ai_summary`,
      () => {
        return { total: 14, by_type: { model: 14 }, aibom_supported: false };
      }
    );
    ...
    assert
      .dom('[data-test-sbomReportList-reportGenerateTitle]', reportList[1])
      .hasText(t('sbomModule.aiBomDownloadPdfPrimaryText'));
  });
});
