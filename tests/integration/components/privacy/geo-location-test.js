...
module('Integration | Component | privacy/geo-location', function (hooks) {
  ...
  hooks.beforeEach(async function () {
    ...
    const abc = this.server.create('geo-location', {
      countryCode: 'IN',
      country_name: 'India',
  ...
  test('it has complete flow existssss', async function (assert) {
    await render(
      hbs(`<PrivacyModule::AppDetails::GeoLocation @file={{this.file}}/>`)
    );
    ...
    // const knowMoreBtn = document.querySelector('#show-more-btn');
    // assert.ok(knowMoreBtn, '"Know More" button is visible');

    // await new Promise((resolve) => setTimeout(resolve, 50000)); // wait 500 ms
