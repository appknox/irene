...
module('Integration | Component | admin/mfa', function (hooks) {
  ...
  test('it should render', async function (assert) {
    await render(
      hbs`<Admin::Mfa @organization={{this.organization}} @user={{this.user}} />`
    );

    assert
      .dom('[data-test-admin-common-page-header-title]')
