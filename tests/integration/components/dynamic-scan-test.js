...
module('Integration | Component | dynamic-scan', function (hooks) {
  ...
  test.each(
    ...
    async function (assert, status) {
      ...
      if (this.file.dynamicStatus === ENUMS.DYNAMIC_STATUS.ERROR) {
        assert.dom('[data-test-dynamicScan-startBtn]').hasText('t:errored:()');
  ...
  test.each(
    ...
    async function (assert, { automatedScan }) {
      ...
      assert
        .dom(apiFilterRows[0].querySelectorAll('[data-test-apiFilter-cell]')[0])
        .hasText('api.example.com');

      assert
        .dom(apiFilterRows[1].querySelectorAll('[data-test-apiFilter-cell]')[0])
  ...
  test('test when preferred device is not available', async function (assert) {
    ...
    assert
      .dom(`[data-test-projectPreference-deviceTypeSelect] .${classes.trigger}`)
      .hasClass(classes.triggerError);

    assert
      .dom(`[data-test-projectPreference-osVersionSelect] .${classes.trigger}`)
      .hasClass(classes.triggerError);
