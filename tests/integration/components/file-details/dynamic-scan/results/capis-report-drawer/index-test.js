...
module(
  ...
  function (hooks) {
    ...
    test.each(
      'it should disable generate button if dynamic scan is running',
      [
        {
      ...
      async function (assert, { automatedStatus, manualStatus }) {
        ...
        const generateReportTooltipContainer = find(
          '[data-test-fileDetailsDynamicScan-capis-generateReportCTA-tooltipContainer]'
        );

        await triggerEvent(generateReportTooltipContainer, 'mouseenter');

        assert
          .dom(
            '[data-test-fileDetailsDynamicScan-capis-generateReportCTA-tooltipContent]'
          )
          .hasText(t('apiScanModule.cannotGenerateReportFiles'));

        await triggerEvent(generateReportTooltipContainer, 'mouseleave');
      }
    );
