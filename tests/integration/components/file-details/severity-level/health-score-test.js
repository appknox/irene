...
const selectors = {
  ...
  skeleton: '[data-test-fileDetailSeverityLevel-healthScore-skeleton]',
...
module(
  ...
  function (hooks) {
    ...
    test('detailed variant renders score, max-score, status chip and scoring-details button', async function (assert) {
      ...
      assert
        .dom(selectors.scoringDetailsBtn)
        .containsText(t('healthScore.scoringDetails'));
    });
    ...
    test('compact variant renders section title above the score box', async function (assert) {
      ...
      assert.dom(selectors.title).hasText(t('healthScore.title'));
      assert.dom(selectors.score).hasText('60');
      assert.dom(selectors.maxScore).hasText('/100');
      assert
        .dom(selectors.statusChip)
        .hasText(t('healthScore.scoringMethodology.ratingGood'));
      assert
        .dom(selectors.scoringDetailsBtn)
        .containsText(t('healthScore.scoringDetails'));
    });
    ...
    test.each(
      'status chip shows the translated label for each status',
      [
      ...
      async function (assert, [status, translationKey]) {
        setupHealthScoreEndpoint(this.server, {
          knoxiq_enabled: false,
          score: 50,
          score_type: 'severity_based',
        ...
        assert.dom(selectors.statusChip).hasText(t(translationKey));
      }
    );
    ...
    test('clicking scoring details button opens the drawer', async function (assert) {
      ...
      assert.dom(selectors.drawerTitle).hasText(t('healthScore.scoringAudit'));
    });
