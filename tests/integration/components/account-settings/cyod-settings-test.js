...
const selectors = {
  ...
  downloadBtnMac: '[data-test-cyodSettings-downloadBtnMac]',
  downloadBtnWindows: '[data-test-cyodSettings-downloadBtnWindows]',
  downloadBtnLinux: '[data-test-cyodSettings-downloadBtnLinux]',
...
module(
  ...
  function (hooks) {
    ...
    test('the drawer titles each of the five setup steps in order', async function (assert) {
      ...
      assert
        .dom('[data-test-cyodSettings-downloadBtnMac]')
        .hasAttribute('href', /Mercer-x86_64\.dmg$/);

      assert
        .dom('[data-test-cyodSettings-downloadBtnWindows]')
        .hasAttribute('href', /Mercer-windows-x64\.exe$/);

      assert
        .dom('[data-test-cyodSettings-downloadBtnLinux]')
        .hasAttribute('href', /mercer\.deb$/);
    ...
    test('the drawer offers the Mercer download and the server URL to copy', async function (assert) {
      ...
      assert
        .dom(selectors.downloadBtnMac)
        .hasText(t('cyod.settings.downloadMercerMac'))
        .hasAttribute('href', /Mercer-x86_64\.dmg$/)
        .hasAttribute('target', '_blank')
        .hasAttribute('rel', 'noopener noreferrer');

      assert
        .dom(selectors.downloadBtnWindows)
        .hasText(t('cyod.settings.downloadMercerWindows'))
        .hasAttribute('href', /Mercer-windows-x64\.exe$/);

      assert
        .dom(selectors.downloadBtnLinux)
        .hasText(t('cyod.settings.downloadMercerLinux'))
        .hasAttribute('href', /mercer\.deb$/);
    ...
    test('it opens the five-step Mercer setup drawer', async function (assert) {
      ...
      assert.dom(selectors.downloadBtnMac).exists();
      assert.dom(selectors.downloadBtnWindows).exists();
      assert.dom(selectors.downloadBtnLinux).exists();
