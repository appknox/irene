...
module(
  ...
  function (hooks) {
    ...
    const ContextClass = NotificationMap['NF_NSREQSTD1'].context;
    ...
    test('it should show approved state if namespace is approved and you if approver is current user', async function (assert) {
      ...
      await render(hbs`<NotificationsPage::Messages::NfNsreqstd1 @notification={{this.notification}}
      @context={{this.context}}/>`);

      assert.dom('[data-test-nf-nsreqstd1-primary-message]').hasText(
        t('notificationModule.messages.nf-nsreqstd1.primary', {
          requester_username: 'appknox_requester',
          platform_display: 'android',
          namespace_value: 'com.mfva.test',
        })
      ...
      assert.dom('[data-test-nf-nsreqstd1-approved-message]').hasText(
        t('notificationModule.messages.nf-nsreqstd1.approved', {
          moderaterName: 'You',
        })
      );
      assert.dom('[data-test-nf-nsreqstd1-approved-icon]').exists();
    });
    ...
    test('it should show rejected state if namespace is rejected', async function (assert) {
      ...
      assert
        .dom('[data-test-nf-nsreqstd1-rejected-message]')
        .hasText(t('notificationModule.messages.nf-nsreqstd1.rejected'));

      assert.dom('[data-test-nf-nsreqstd1-rejected-icon]').exists();
    });
    ...
    test('it should show approve and reject for unmoderated namespace', async function (assert) {
      ...
      assert.dom('[data-test-nf-nsreqstd1-approve-button]').exists();

      assert.dom('[data-test-nf-nsreqstd1-reject-button]').exists();

      assert
        .dom('[data-test-nf-nsreqstd1-link]')
        .exists()
    ...
    test('it should approve namespace when approve is clicked for unmoderated namespace', async function (assert) {
      ...
      await click('[data-test-nf-nsreqstd1-approve-button]');

      assert.dom('[data-test-nf-nsreqstd1-approve-button]').doesNotExist();
      assert.dom('[data-test-nf-nsreqstd1-reject-button]').doesNotExist();
    ...
    test('it should reject namespace when reject is clicked for unmoderated namespace', async function (assert) {
      ...
      await click('[data-test-nf-nsreqstd1-reject-button]');
