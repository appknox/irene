...
module(
  ...
  function (hooks) {
    ...
    test('it disables delete for the active cert while siblings exist', async function (assert) {
      ...
      // The button is disabled, so the reason comes from the tooltip wrapper
      // rather than a title on the button itself.
      await triggerEvent(findAll(selectors.deleteTooltip)[0], 'mouseenter');
