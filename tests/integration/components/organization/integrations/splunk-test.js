...
const assertNonIntegratedUI = async (assert) => {
  assert
    .dom('[data-test-integration-card-description="Splunk"]')
  ...
  assert
    .dom('[data-test-orgIntegrations-splunkAccount-disconnectBtn]')
    .doesNotExist();
};

const assertIntegratedUI = async (assert, splunkIntegrationInfo) => {
  assert.dom('[data-test-integration-card-description="Splunk"]').exists();
  ...
  function (hooks) {
    ...
    test.each(
      ...
      async function (assert, { fail }) {
        ...
        assert
          .dom('[data-test-orgIntegrations-splunk-drawer-title]')
    ...
    test.each(
      ...
      async function (assert, { fail, error, errorMsg }) {
        ...
        assert
          .dom('[data-test-orgIntegrations-splunkAccount-saveBtn]')
          .exists()
          .isNotDisabled();

        await click('[data-test-orgIntegrations-splunkAccount-saveBtn]');
