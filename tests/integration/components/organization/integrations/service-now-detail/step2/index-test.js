...
module(
  ...
  function (hooks) {
    ...
    test('it shows a loader and hides mapping rows while columns are being fetched', async function (assert) {
      this.setProperties({
        selectedSNTable: SN_TABLE_ITEMS[2],
        isCustomTable: true,
        selectedCustomTable: { label: 'Security Incident', value: 'u_security_incident' },
        fetchTableColumnsIsRunning: true,
      });
      ...
      assert.dom().doesNotContainText(t('serviceNow.columnMapping'));
    });

    test('it shows field mapping notes when a custom table is selected and columns are loaded', async function (assert) {
      const customTable = this.server.create('sn-custom-table');
      const columns = this.server.createList('sn-column', 2);

      const selectedCustomTable = { label: customTable.label, value: customTable.value };
      const snColumns = columns.map(({ label, value, type }) => ({ label, value, type }));

      this.setProperties({
        selectedSNTable: SN_TABLE_ITEMS[2],
        isCustomTable: true,
        selectedCustomTable,
        availableColumnsFor: { vulnerability_name: snColumns, risk: snColumns },
      });

      await render(TEMPLATE);

      assert.dom().containsText(t('serviceNow.fieldMappingNote1'));
      assert.dom().containsText(t('serviceNow.fieldMappingNote2'));
      assert.dom().containsText(t('serviceNow.fieldMappingNote3'));
    });
