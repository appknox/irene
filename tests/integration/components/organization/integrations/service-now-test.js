...
const assertNonIntegratedUI = async (assert) => {
  assert
    .dom('[data-test-integration-card-description="ServiceNow"]')
    .hasText(t('serviceNowIntegrationDesc'));

  assert
    .dom('[data-test-integration-card-connectBtn]')
    .isNotDisabled()
    .hasText(t('connect'));

  await click('[data-test-integration-card-connectBtn]');
  ...
  assert
    .dom('[data-test-orgIntegrations-serviceNowAccount-saveBtn]')
    .isNotDisabled()
    .hasText(t('save'));
  ...
  assert
    .dom('[data-test-orgIntegrations-serviceNowAccount-disconnectBtn]')
    .doesNotExist();
};

const assertIntegratedUI = async (assert, sNIntegrationInfo) => {
  assert.dom('[data-test-integration-card-description="ServiceNow"]').exists();

  assert
    .dom('[data-test-integration-card-manageBtn]')
    .isNotDisabled()
    .hasText(t('manage'));
...
module(
  ...
  function (hooks) {
    ...
    test.each(
      ...
      async function (assert, { fail }) {
        ...
        const disconnectBtn = find(
          '[data-test-orgIntegrations-serviceNowAccount-disconnectBtn]'
        );
        ...
        assert
          .dom('[data-test-orgIntegrations-serviceNow-drawer-title]')
    ...
    test('it should validate ServiceNow inputs', async function (assert) {
      ...
      await click('[data-test-orgIntegrations-serviceNowAccount-saveBtn]');
