...
module(
  ...
  function (hooks) {
    ...
    // ─── Validation ──────────────────────────────────────────────────────────
    test.each(
      'the save button stays disabled while risk or criteria is missing',
      [
        // [selectRisk, selectCriteria]
        [false, false],
        [true, false],
        [false, true],
      ],
      async function (assert, [selectRisk, selectCriteria]) {
        this.dataModel = this.buildDataModel();
    ...
    test('clearing a filled comment shows the validation error and disables save', async function (assert) {
      this.dataModel = this.buildDataModel();
      ...
      await fillIn(selectors.commentInput, '');

      assert
        .dom(selectors.commentInputError)
    ...
    test('a whitespace-only comment disables save without a changeset validation error', async function (assert) {
      this.dataModel = this.buildDataModel();
    ...
    test('the save button is disabled until the form is changed', async function (assert) {
      this.dataModel = this.buildDataModel();
    ...
    test('editing an existing override returns to the override details view', async function (assert) {
      ...
      await fillIn(selectors.commentInput, 'Still an accepted risk');
      await click(selectors.saveBtn);

      assert.deepEqual(this.saveCalledWith, [
        ENUMS.RISK.LOW,
        'Still an accepted risk',
        false,
      ]);
    ...
    test('a field-error payload notifies the field name and message', async function (assert) {
      this.dataModel = this.buildDataModel({
        editSaveOverrideHandler: () => {
          const error = new Error('Validation failed');

          error.payload = { comment: ['This field may not be blank.'] };
      ...
      assert.strictEqual(
        ...
        'shows the field-prefixed error notification'
      );
