...
function setRoles(test, { isAdmin = false, isOwner = false } = {}) {
  const me = test.owner.lookup('service:me');

  me.org.is_admin = isAdmin;
  me.org.is_owner = isOwner;
}
...
module(
  ...
  function (hooks) {
    ...
    test.each(
      'it renders action chips based on owner/admin role and finding state',
      [
        {
        ...
        {
          name: 'owner + failed → failed-edit chip only',
          roles: { isOwner: true },
          attrs: { passed: false, is_overridden: false },
          expectedChips: ['failed-edit'],
          notExpectedChips: ['override-description'],
        ...
        {
          name: 'owner + overridden → override-description chip only',
          roles: { isOwner: true },
          attrs: { passed: true, is_overridden: true },
          expectedChips: ['override-description'],
          notExpectedChips: ['failed-edit'],
        ...
        {
          name: 'member + overridden → no chips',
          roles: {},
          attrs: { passed: true, is_overridden: true },
          expectedChips: [],
          notExpectedChips: ['failed-edit', 'override-description'],
      ...
      async function (
        assert,
        { roles, attrs, expectedChips, notExpectedChips }
      ) {
        setRoles(this, roles);
