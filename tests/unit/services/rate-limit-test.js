...
module('Unit | Service | rate-limit', function (hooks) {
  ...
  test('it handles rate limit response', function (assert) {
    ...
    const mockResponse = {
      lock_time: '30.5',
      detail: 'Rate limit exceeded. Try again in 30.5 seconds',
    };
    ...
    assert.strictEqual(service.remainingTime, 30);

    assert.strictEqual(notify.errorMsg, `${t('rateLimitExceeded')} 30s`);
