...
module('Unit | Helper | device type', function () {
  test('it works', function (assert) {
    ...
    assert.equal(
      deviceType([ENUMS.DS_AUTOMATED_DEVICE_TYPE.NO_PREFERENCE]),
      'anyDevice',
      'No Preference'
    );

    assert.equal(
      deviceType([ENUMS.DS_AUTOMATED_DEVICE_TYPE.PHONE_REQUIRED]),
      'phone',
      'Phone'
    );

    assert.equal(
      deviceType([ENUMS.DS_AUTOMATED_DEVICE_TYPE.TABLET_REQUIRED]),
      'tablet',
      'Tablet'
    );
  });
