...
module('Unit | Model | sbom-component-inventory', function (hooks) {
  ...
  test('cleanVersion and cleanLatestVersion strip quotes and whitespace', function (assert) {
    const store = this.owner.lookup('service:store');
    ...
    assert.strictEqual(model.cleanVersion, '4.4.3');
    assert.strictEqual(model.cleanLatestVersion, '4.13.2');
  });
