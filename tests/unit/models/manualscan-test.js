...
module('Unit | Model | manualscan', function (hooks) {
  ...
  test('it exists', function (assert) {
    ...
    const manualscan = this.owner
      .lookup('service:store')
      .createRecord('manualscan');
    ...
    assert.strictEqual(manualscan.get('filteredAppAction'), 0, 'App Action/0');

    manualscan.set('appAction', 1);

    assert.strictEqual(manualscan.get('filteredAppAction'), 1, 'App Action/1');
    ...
    assert.strictEqual(manualscan.get('loginStatus'), 'no', 'Login Status/No');
