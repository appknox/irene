...
module('Unit | Model | project', function (hooks) {
  ...
  test('it exists', function (assert) {
    const project = this.owner.lookup('service:store').createRecord('project');

    assert.equal(
      project.get('pdfPassword'),
      'Unknown!',
      'PDF Password/Unknown'
    );
    ...
    assert.equal(
      project.get('platformIconClass'),
      'mobile',
      'Platform Icon Class/mobile'
    );
