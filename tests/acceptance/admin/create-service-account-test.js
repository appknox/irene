...
module('Acceptance | Create service account', function (hooks) {
  ...
  test.each(
    ...
    async function (assert, { duplicate }) {
      ...
      await visit(
        '/dashboard/admin/service-account/create'.concat(
  ...
  test.each(
    ...
    async function (assert, { duplicate }) {
      ...
      assert.strictEqual(
        currentURL(),
        `/dashboard/admin/service-account/details/${this.createdServiceAccount.id}`
      );
  ...
  test('it shows validation error for name & description', async function (assert) {
    ...
    await visit('/dashboard/admin/service-account/create');
