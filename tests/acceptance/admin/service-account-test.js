...
module('Acceptance | Organization Service Account List', function (hooks) {
  ...
  test.each(
    ...
    async function (assert, showSystemCreated) {
      ...
      let url = '/dashboard/admin/service-account';
  ...
  test('test show system created checkbox change', async function (assert) {
    ...
    await visit('/dashboard/admin/service-account');
    ...
    assert.strictEqual(
      currentURL(),
      '/dashboard/admin/service-account?show_system_created=true'
    );
    ...
    assert.strictEqual(currentURL(), '/dashboard/admin/service-account');
  ...
  test('it should navigate to service account details page', async function (assert) {
    ...
    assert.strictEqual(
      currentURL(),
      `/dashboard/admin/service-account/details/${serviceAccounts[0].id}`
    );
  });
  ...
  test.each(
    ...
    async function (assert, { featureNotEnabled, userHasNoAccess }) {
      ...
      assert.strictEqual(
        currentURL(),
        featureNotEnabled
          ? '/dashboard/admin/user-management/users'
          : '/dashboard/projects'
      );
    }
  );
