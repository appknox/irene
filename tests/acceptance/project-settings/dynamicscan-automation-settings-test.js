...
module(
  ...
  function (hooks) {
    ...
    test('AI DAST disabled + super user: shows both V1 and V2 scenarios, scan window, and "Super User Only" label', async function (assert) {
      ...
      assert.dom(selectors.scenariosV2HeaderText).containsText('v2');
    });

    test('AI DAST enabled: shows only V2 scenarios and scan window; hides V1 and "v2" suffix', async function (assert) {
      ...
      assert.dom(selectors.scanWindowRoot).exists('scan window shown');
      assert.dom(selectors.scenariosV2HeaderText).doesNotContainText('v2');
    });

    test('AI DAST enabled + super user: shows both V1 and V2 scenarios; superUser branch takes precedence', async function (assert) {
      this.currentOrganizationMe.update({ has_security_permission: true });
      ...
      assert
        .dom(selectors.scenariosV1Root)
        .exists('V1 scenarios section shown for super user');

      assert
        .dom(selectors.scenariosV2Root)
        .exists('V2 scenarios section shown for super user');
    ...
    test('AI DAST enabled + super user: V1 scenario route is accessible', async function (assert) {
      this.currentOrganizationMe.update({ has_security_permission: true });
      ...
      assert.strictEqual(
        currentURL(),
        `/dashboard/project/${this.project.id}/settings/dast-automation-scenario/${scenario.id}`,
        'super user stays on V1 scenario route even when AI DAST is enabled'
      );
    });
