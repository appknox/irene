...
module('Acceptance | landing page', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    assert.dom('[data-test-landingPage]').exists();

    assert
      .dom('[data-test-landingPage-title]')
      .exists()
      .hasText(t('toSecureYourMobileApps'));

    assert.dom('[data-test-landingPage-appLogo]').exists();

    assert
      .dom('[data-test-landingPage-logoutBtn]')
      .isNotDisabled()
      .hasText(t('logout'));

    assert.dom('[data-test-landingPage-productCard]').exists({
      count: 2,
    });
  });
  ...
  test('it redirects to appknox dashboard', async function (assert) {
    ...
    assert
      .dom('[data-test-landingPage-productCard-title]')
      .exists({ count: 2 });

    const titles = this.element.querySelectorAll(
      '[data-test-landingPage-productCard-title]'
    );
    ...
    const links = this.element.querySelectorAll(
      '[data-test-landingPage-productCard-link]'
    );

    assert.dom('[data-test-landingPage-productCard-icon-vp]').exists();

    assert.dom('[data-test-landingPage-productCard-icon-sm]').exists();
