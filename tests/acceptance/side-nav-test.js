...
module('Acceptance | side nav test', function (hooks) {
  ...
  test.each(
    ...
    [
      { expectedUrl: '/dashboard/storeknox', visitUrl: '/dashboard/projects' },
      { expectedUrl: '/dashboard/projects', visitUrl: '/dashboard/storeknox' },
    ],

    async function (assert, urls) {
      await visit(urls.visitUrl);
      ...
      assert.strictEqual(currentURL(), urls.expectedUrl);
    }
  );
});
